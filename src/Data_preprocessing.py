...
st.write("""
Предварительная обработка важный этап в машинном обучении. Для того, чтобы обучать модель, ей необходимо предоставить очищенные данные в понятном виде. Ни одну модель не обучают на данных, 
которые до этого не обработали. Именно предварительная обработка, на равне с разведочным анализом данных, обычно занимает большую часть проекта. Все те же этапы (и не только они), которые 
применяются здесь, применимы ко всем наборам данных. Именно поэтому данная лабораторная работа будет актуальна для специалистов из любых сфер, которые хотят познакомиться и освоить возможности машинного обучения.
...
\nЛабораторная работа состоит из 3 блоков:
...
st.image(image='../images/Pipeline_1.png', caption='Схема (пайплайн) лабораторной работы', use_column_width='auto')

#-------------------------О проекте-------------------------
pipeline_expander = st.expander("Описание пайплайна стримлита:")
...
exp = st.expander('Какие выводы можно сделать?')
exp.markdown("""
Предобработка данных - важный и долгий процесс в машинном обучении. Группировка похожих данных, очищение от пропусков, генерация новых признаков на основе старых, обработка ошибок - все это
помогает получить полную информацию при анализе данных и получить наиболее точную модель при обучении. Результаты моделей напрямую зависят от того, какие преобразования будут произведены перед обучением.
Чтобы на примере в этом убедиться, можно пройти лабораторную работу [**Влияние обработки данных на точность прогноза**](https://darealmumba-ranepa-data-preprocessi-data-preprocessing-2-j07j4m.streamlitapp.com).
""")
